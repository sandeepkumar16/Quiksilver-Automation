...
public class TestListenerFailPass extends TestListenerAdapter {
	...
	driver = WebDriverManager.getDriverInstance();
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Could not take screenshot on success"+ tr.getInstance());//getInstanceName =package+className

			log.debug("Could not take screenshot on success"+ tr.getInstance());//getInstanceName =package+className
